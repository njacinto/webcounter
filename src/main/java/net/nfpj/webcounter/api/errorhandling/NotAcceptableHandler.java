...
public class NotAcceptableHandler implements ExceptionMapper<NotAcceptableException> {
    private static final Logger log = LoggerFactory.getLogger(NotAcceptableHandler.class);

    @Override
    public Response toResponse(NotAcceptableException exception) {
        log.warn("Not Acceptable error catched: " + exception.getMessage(), exception);
        //
        MessageResponse response = new MessageResponse("");
		return Response.status(NOT_ACCEPTABLE).entity(response).build();
    }
